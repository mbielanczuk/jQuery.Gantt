		...
		var core = {
			...
			leftPanel: function (element) {
				...
				$.each(element.data, function(i, entry) {
					...
					{
						ganttLeftPanel
							.append($('<div class="row name row' + i + ' "/>').append($('<span class="label"/>').html(entry.name)))
							.append($('<div class="row desc row' + i + ' "/>').append($('<span class="label"/>').html(entry.desc)));
					}
				});
			...
			rightPanel: function(element) {
				...
				switch (settings.scale)
				{
					...
					case 'hours':
						...
						for (var i = 0; i < range.length; i++) 
						{
							...
							if (rgetDay != getDay) {
								...
								daysStr += '<div class="row day '+day_class2+'" '
								        ...
								        +  ' <div class="label"/>' + day.getDate() + '</div></div>';

								dowStr += '<div class="row day ' + day_class2 + '" '
								        +  ' style="width: ' + tools.getCellSize() * hoursInDay + 'px;"> '
								        +  ' <div class="label"/>' + settings.dow[getDay] + '</div></div>';
						...
						years.html(years.html()+
							'<div class="row header year" style="width: '
							+ tools.getCellSize()*daysInYear + 'px;"><div class="label"/>'
							+ year 
							+ '</div></div>');
						/*
						 * Last month
						...
						dowStr += '<div class="row day ' + day_class + '" '
						        +  ' style="width: ' + tools.getCellSize()*hoursInDay + 'px;"> '
						        +  ' <div class="label"/>' + settings.dow[day.getDay()] + '</div></div>';
					...
					case 'weeks':
						...
						for (var i = 0; i < range.length; i++) 
						{
							...
							daysStr += '<div class="row day wd" '
							        ...
							        +  ' <div class="label"/>'+rday.getWeekOfYear()+'</div></div>';
						}
					...
					default:
						...
						for (var i = 0; i < range.length; i++) 
						{
							...
							daysStr += '<div class="row day ' + day_class + '" '
							        ...
							        +  ' <div class="label"/>' + rday.getDate() + '</div></div>';
							dowStr += '<div class="row day ' + day_class + '" '
					...
					ganttNavigate = $('<div class="navigate" />')
						.append($('<div class="nav-slider" />')
							.append($('<div class="nav-slider-left" />')
								.append($('<a href="javascript:///" class="nav-link nav-page-back"/>')
									.html('&lt;')
									.click( function () {
								...
								.append($('<a href="javascript:///" class="nav-link nav-page-next"/>')
									.html('&gt;')
									.click( function () {
										core.navigatePage(element, 1);
									}))
								.append($('<a href="javascript:///" class="nav-link nav-now"/>')
									.html('&#9679;')
									.click( function () {
